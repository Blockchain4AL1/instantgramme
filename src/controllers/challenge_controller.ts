      ...
      `SELECT cr.*,
       ...
       (
         SELECT COUNT(*) FROM challenge_result x
          WHERE (CAST(cr.resultat_obtenu AS DECIMAL(6,3) ) < CAST(x.resultat_obtenu AS DECIMAL(6,3))
                OR
                 (cr.temps_execution >= x.temps_execution
            AND CAST(cr.resultat_obtenu AS DECIMAL(6,3) ) <= CAST(x.resultat_obtenu AS DECIMAL(6,3))))
